...
import {
  SvRtFileSchema,
  isSvRtLogDataType,
  isValidPerfThreadName,
  SvRtNodeMemorySchema,
} from "./perfSchemas";
import type {
  SvRtFileType,
  SvRtLogDataType,
...
import {
  PERF_DATA_BUCKET_COUNT,
  PERF_DATA_INITIAL_RESOLUTION,
  PERF_DATA_MIN_TICKS,
} from "./config";
...
export default class SvRuntimeStatsManager {
  ...
  private async collectStats() {
    ...
    if (healthMonitorStatus !== "ONLINE" && healthMonitorStatus !== "PARTIAL")
      return;
    ...
    if (fetchRawPerfDataRes.status === "rejected")
      throw fetchRawPerfDataRes.reason;
    ...
    if (
      ...
      console.verbose.warn(
        "Not enough ticks to log. Skipping this collection."
      );
      return;
    }
    ...
    if (!this.lastPerfBoundaries) {
      ...
      JSON.stringify(perfBoundaries) !== JSON.stringify(this.lastPerfBoundaries)
    ) {
    ...
    if (
      this.lastRawPerfData &&
      didPerfReset(perfMetrics, this.lastRawPerfData)
    ) {
      console.warn(
        "Performance counter reset. Resetting lastPerfCounts/lastPerfSaved."
      );
      this.resetPerfState();
    } else if (
      this.lastRawPerfSaved &&
      didPerfReset(perfMetrics, this.lastRawPerfSaved.data)
    ) {
    ...
    if (convars.debugExternalStatsSource) {
      try {
        const playerCountResp = await got(
          `http://${fxServerHost}/players.json`
        ).json();
    ...
    console.verbose.ok(
      `Collected performance snapshot #${this.statsLog.length}`
    );
  ...
  private async loadStatsHistory() {
    try {
      ...
      if (fileData?.version !== LOG_DATA_FILE_VERSION)
        throw new Error(
          `invalid version ${fileData?.version} - ${LOG_DATA_FILE_VERSION}`
        );
      ...
      console.verbose.debug(
        `Loaded ${this.statsLog.length} performance snapshots from cache`
      );
      await optimizeSvRuntimeLog(this.statsLog);
    } catch (error) {
      if ((error as any)?.code === "ENOENT") {
        console.verbose.debug(
          `${LOG_DATA_FILE_NAME} not found, starting with empty stats.`
        );
        return;
      }
      if (error instanceof ZodError) {
        console.warn(
          `Failed to load ${LOG_DATA_FILE_NAME} due to invalid data.`
        );
      } else {
        console.warn(
          `Failed to load ${LOG_DATA_FILE_NAME} with message: ${
  ...
  private async saveStatsHistory() {
    try {
      ...
      console.warn(
        `Failed to save ${LOG_DATA_FILE_NAME} with message: ${
          (error as Error).message
        }`
      );
    }
  ...
  public getChartData(threadName: string): PerfChartApiResp {
    if (!isValidPerfThreadName(threadName))
      return { fail_reason: "invalid_thread_name" };
    if (!this.statsLog.length || !this.lastPerfBoundaries?.length)
      return { fail_reason: "data_unavailable" };
