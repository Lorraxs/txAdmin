...
const getPublishVersion = (isOptional) => {
  ...
  try {
    ...
    console.log(1, refRemoved);
    const parsedVersion = new SemVer(refRemoved);
    console.log(2, parsedVersion.toString());
    const isPreRelease = parsedVersion.prerelease.length > 0;
    const potentialExpiration = new Date().setUTCHours(
      24 * config.preReleaseExpirationDays,
      0,
      0,
      0
    );
...
class txAdminRunner {
  ...
  spawnServer() {
    ...
    try {
      ...
      console.error(
        "[RUNNER] Failed to start FXServer with the following error:"
      );
      console.dir(error);
      process.exit(1);
    }
...
const runDevTask = async (txVersion, preReleaseExpiration) => {
  ...
  try {
    ({ fxServerRootPath, fxsBinPath, monitorPath } = getFxsPaths(
      process.env.TXADMIN_DEV_FXSERVER_PATH
    ));
  } catch (error) {
    console.error(
      "[BUILDER] Could not extract/validate the fxserver and monitor paths."
    );
    console.error(error);
    process.exit(1);
  }
  ...
  fs.writeFileSync(
    path.join(monitorPath, "package.json"),
    '{"type":"commonjs"}'
  );
...
if (taskType === "dev") {
  ...
  const { txVersion, isPreRelease, preReleaseExpiration } =
    getPublishVersion(false);
